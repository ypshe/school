...
<html>
	...
	<body>
		...
		<div class="header bglan overflow textcenter wbai">
			...
			<h3 class="sanwu bai textcenter">单选错题库</h3>
		...
		<article>
			...
			<div class="yanzhengbo top20">
				<div class="title">
					...
					<button class="fr h3 liu" style="margin-top: 1.001vw" onclick="javascript:location.href='/wap/user/errorExam/2'">查看多选错题</button>
				</div>
				@foreach($exam as $k=>$v)
				<div style="padding:0 2.4vw" class="shiti top20 bgbai">
					<table class="wbai top20">
						...
						<tbody>
							...
							<tr>
								<td colspan="3">
									<a class="fl cuoti" href="javascript:;" onclick="cuoti({{$v->id}},'{{$v->error}}')">查看错题</a>
									{{--<a class="fl laiyuan" href="javascript:;" style="margin-left: 6vw;" onclick="laiyuan({{$v->id}})">查看来源</a>--}}
									<a class="fl" href="/wap/user/delError/1/{{$v->id}}" style="margin-left: 6vw;">删除错题</a>
								</td>
							</tr>
						</tbody>
					</table>
		...
		<script>
			...
			//查看错题弹窗
			//查看来源
			function laiyuan(value){
				var laiyuan = '<div class="cuotilook bgbai" id="laiyuan" style="min-height: 71.3vw;posiation:relative">'+
				'<i class="iconfont icon-error guanbi jiu h1"></i>'+
			'<div class="title">'+
				'<span class="fl h2">查看来源</span>'+
			'</div>'+
			'<div class="laiyuanb top40">'+
				'<table>'+
					'<thead>'+
						'<td class="h2 san" style="width:65.3vw">'+
							'试卷名称'+
						'</td>'+
