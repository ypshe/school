	...
	<body class="bgbai">
		...
		<article>
			...
			<div class="onlinelianxi bgbai"  style="position: fixed;top:14.6vw;width:100%;left:0px;">
				<h1 class="h2 san textcenter">{{$pro->name}} 在线练习</h1>
		...
		<script>
            ...
            function submitExam(time){
                ...
                $.ajax({
                    type:'post',
                    url:'{{url('/ajax/wap/user/submitTest')}}',
                    dataType:'json',
                    data:data,
                    success:function(res){
                        if(res.error===1){
                            layer.open({
                                content: '您未作答任何题目，本次练习0分无效！！！'
                            });
                        }else if(res.error===2){
                            layer.open({
                                content: '操作有误，请重新答题！！！'
                            ...
                            $('#chakanshijuan').attr('href','{{url('/wap/user/seeTest')}}'+'/'+res.id);
                        }
                        $(".shijuantan").show();
                        $(".zhe").show();
                    }
