...
<div class="main">
	<div class="kaoshile">
		<form action="" id="examForm">
			...
			<div class="kaoshixinxi">
				练习人员：{{$user->name}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;/&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;试卷总分：100分&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;/&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;练习时长：{{$pro->exam_time}}分钟
			</div>
			<div class="zhuyi">
				注：答题过程中请不要刷新页面
			</div>
	...
	<div class="kaoshiri" style="position: fixed;top:126px;right:50%;margin-right:-600px;">
...
<script>
    ...
    function timedCount() {
        ...
        if(time>0){
            if(time===5*60){
                layer.msg('练习只剩下最后5分钟！请快速答题后提交，系统将在5分钟后自动提交试卷！');
            }
    ...
    function submitExam(time){
        ...
        $.ajax({
            type:'post',
            url:'{{url('/user/submitTest')}}',
            dataType:'json',
            data:data,
            success:function(res){
                if(res.error===1){
                    $('#kaoshichengji').html('您未作答任何题目，本次练习0分无效！！！');
                    $('#chakanshijuan').hide();
                }else{
                    $('#score').html(res.value);
                    $('#useTime_m').html(parseInt(res.time/60));
                    $('#useTime_s').html(parseInt(res.time%60));
                    $('#chakanshijuan').attr('href','{{url('/user/seeTest')}}'+'/'+res.id);
                }
                $(".shijuantan").show();
                $(".zhe").show();
            }
