...
@section('content')
	...
	<div class="lianxitan">
		...
		<h1></h1>
		<span style="color:#e70a2e;margin-top:32px;font-size:16px;"><b style="color:#03478f;font-weight: 400;">练习人员：</b><name></name></span>

		<div class="lianxitant">
			<span>试卷总分：<b>100分</b></span>
			<span>及格分数：<b>60分</b></span>
			<span>最高得分：<b>100分</b></span>
			<span>练习时间：<b id="exam_time"></b>分钟</span>
	...
	<div class="main">
		...
		<div class="mianbao">
			...
			<span>></span>
			<a href="{{url('/user/online_exam')}}">在线练习</a>
		</div>
		<!--面包屑导航-->
		<div class="gerenbox">
			@include('user_public')
			<div class="gerenbori" style="min-height: 847px;">
				...
				<div class="shuoming">
					<dd>温馨提示：</dd>
					<dt>1：只能参加已完成学时要求的专业练习；</dt>
					<dt>2：总分100分，≥60分及格；</dt>
					<dt>3：不限制补考次数，历次练习成绩可在“练习历史”中查询。</dt>
				</div>
				<div class="lianxi">
					<div class="lianxitop">
						<span class="on">练习列表</span>
						<span>练习历史</span>
					</div>
					<!--练习列表-->
						...
						@foreach($pro as $k=>$v)
							<dl>
								...
								<dd>
									<div class="ddle">
										<h3>{{$v->name}}</h3>
										{{--<span>练习期限：2017-04-18 9：00到2018-12-31 00：00</span>--}}
										<span></span>
										<span>练习时间：<span id="kaoshishijian{{$v->id}}" style="display: inline;">{{$v->exam_time}}</span>分钟</span>
									</div>
						...
						@foreach($history as $k=>$v)
							<dl>
								...
								<dd>
									<div class="ddle">
										<h3>{{$v->pname}}</h3>
										<span>练习时间：{{$v->time}}</span>
										<span>成绩：<b>{{$v->value}}</b>分</span>
									</div>
									<div class="ddri chakan">
										<a href="{{url('/user/seeTest/'.$v->id)}}">查看</a>
									</div>
								</dd>
							</dl>
						@endforeach
	...
	<script>
        ...
        $(".jinru").click(function(){
            ...
            $(".lianxitan h1").text(biaoti+" 在线练习");
            $(".lianxitan name").text('{{$user->name}}');
            $(".lianxitanm a").attr('href','{{url('/user/test')}}'+'/'+$(this).attr('pid'));
            $(".lianxitan").show();
            $(".zhe").show();
