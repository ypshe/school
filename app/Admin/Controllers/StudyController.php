    ...
    protected function form($id=0)
    {
        return Admin::form(Study::class, function (Form $form)use($id) {
            ...
            $form->select('tid','选择授课教师')
                ...
                ->setWidth(3);
            $form->text('grade', '课程评分')
                ->rules('required|regex:/^\d+\.(\d{1})$/',[
                    'regex' => '请输入正确的课程评分，如：9.8',
                    'required'=>'请输入课程评分'
                ])->placeholder('请输入课程评分，格式小于10的一位小数，如9.8');
            $form->text('sort', '课程权重')
                ->rules('required|regex:/^\d+$/',[
                    'regex' => '请输入正确的课程权重，如：3',
                    'required'=>'课程权重'
                ])->placeholder('请输入课程权重，权重越大排列越靠前,格式为整数');
            if($id){
